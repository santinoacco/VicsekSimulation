...
module Sim_Vicsek
  ...
  contains
    ...
    ! ============================== !
        ...
        ! el problema al tomar la raiz es si el argumento me da negativo
        ! v(i,1)= dv(i,1)*(1+G*sqrt(V0/real(denom(i))-1))/real(denom(i))   
        ! v(i,2)= dv(i,2)*(1+G*sqrt(V0/real(denom(i))-1))/real(denom(i))
        !pruebo sin raiz
        ! v(i,1)= dv(i,1)*(1+G*(V0/real(denom(i))-1))/real(denom(i))   
        ! v(i,2)= dv(i,2)*(1+G*(V0/real(denom(i))-1))/real(denom(i))

        !pruebo multiplicando por NNbrs(i)... resultado: igual a veces falla
        ! v(i,1)= dv(i,1)*(1+G*sqrt(V0*NNbrs(i)/real(denom(i))-1))/real(denom(i))   
        ! v(i,2)= dv(i,2)*(1+G*sqrt(V0*NNbrs(i)/real(denom(i))-1))/real(denom(i))


! to normalize 'dv' we calculate its module 'denom'
      ! dvmod = sqrt(dv(:,1)**2+dv(:,2)**2)
      ...
      !   enddo
      ! write(*,*)'dv',dv(1,:)
      ! write(*,*)'dv modulo',dvmod(1)

              ! dv(i,1)=dv(i,1)/(V0*denom(i))!?
        ! dv(i,2)=dv(i,2)/(V0*denom(i))
        ! dv(i,1)=dv(i,1)/denom(i)
        ! dv(i,2)=dv(i,2)/denom(i)

        ! !++++++++++++++++ esto es como lo hace Ginelli ++++++++++++++++!
        ! denominador(i) = sqrt((dv(i,1) + eta*NNbrs(i)*psix(i))**2 + (dv(i,2) + eta*NNbrs(i)*psiy(i))**2)
        ! write(*,*) dv(i,1) + eta*NNbrs(i)*psix(i)
        ! if (denominador(i)==0) then
        !   write(*,*) 'denominador dio 0 en paso:', i
        !   stop
        ! end if

        ! dv(i,1)= (dv(i,1) + eta*NNbrs(i)*psix(i))/real(denominador(i))
        ! dv(i,2)= (dv(i,2) + eta*NNbrs(i)*psiy(i))/real(denominador(i))
        !++++++++++++++++  hasta acá ++++++++++++++++!

        ! v(i,1)= dv(i,1)/real(denom(i))
        ! v(i,2)= dv(i,2)/real(denom(i))

        ! write(*,*) v(i,1),v(i,2)
        ! cond = sqrt(v(i,1)**2+v(i,2)**2)
        ! if (cond > 10*V0) then
        !   stop
        ! endif